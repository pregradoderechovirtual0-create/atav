...
export const TIPOS_AUSENTISMO = [
  {
    id: 'cita-medica',
    label: 'Cita médica',
    description: 'Consultas y procedimientos médicos',
  },
  {
    id: 'calamidad',
    label: 'Calamidad familiar',
    description: 'Situaciones familiares urgentes',
  },
  {
    id: 'diligencia',
    label: 'Diligencia de trabajo',
...
export const TIPOS_REPROGRAMACION = [
  ...
  {
    id: 'grabada',
    label: 'Encuentro grabado',
    description:
      'Material disponible para consulta posterior',
  },
] as const
...
const reprogramacionResumen = (data: Record<string, any>) => {
  ...
  const tipo = labelTipoReprogramacion(
...
export const mapDocSolicitud = (
  ...
  const {
    ...
    data.fecha_creacion || data.creadoEn
  )


  const estado = mapEstadoDocente(
    data.estado || 'creada'
  )
...
export async function fetchSolicitudesDocente(
  uid: string
): Promise<SolicitudDocente[]> {
...
export async function crearSolicitudDocente(
  ...
  if(!input.tipo_ausentismo?.trim())
    throw new Error(
      'Selecciona el tipo de ausentismo.'
    )
  ...
  if(!input.fecha_inicio || !input.fecha_fin)
    throw new Error(
      'Completa las fechas de inicio y fin.'
    )
  ...
  if(!input.materia_codigo?.trim())
    throw new Error(
      'Selecciona el encuentro afectado.'
    )
  ...
  if(!input.tipo_reprogramacion?.trim())
    throw new Error(
      'Selecciona el tipo de reprogramación.'
    )
  ...
  if(!fechas.length)
    throw new Error(
      'Indica al menos una fecha de reprogramación.'
    )



  if(
    !validarMaximoDosSemanas(
      input.fecha_inicio,
      fechas
    )
  ){
    ...
    throw new Error(
      'La reprogramación no puede superar dos semanas desde la fecha del encuentro.'
    )
  ...
  const payload={
    ...
    docente_nombre:
      (input.docente_nombre || '').trim(),


    tipo_ausentismo:
      input.tipo_ausentismo.trim(),
    ...
    tipo_reprogramacion:
      input.tipo_reprogramacion.trim(),
...
export function eventosCalendarioDesdeSolicitud(
  sol:SolicitudDocente
){
