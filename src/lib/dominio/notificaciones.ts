...
export interface NotificacionData {
  ...
  titulo: string;
  mensaje: string;
  tipo: TipoNotificacion | "inasistencia_docente";
  leida: boolean;
  ruta: string | null;
  ...
  fechaInicio: string;
  fechaFin: string;
  fechasReprogramacion: string[];
  tipoReprogramacion: string;
  fecha: string;
  fechaSort: number;
}
...
export const mapNotificacionDoc = (
  id: string,
  ...
  return {
    ...
    materiaCodigo: (data.materia_codigo as string) || "",
    solicitudId: (data.solicitud_id as string) || "",
    docenteNombre: (data.docente_nombre as string) || "",
    tipoAusentismo: (data.tipo_ausentismo as string) || "",
    fechaInicio: (data.fecha_inicio as string) || "",
    fechaFin: (data.fecha_fin as string) || "",
    fechasReprogramacion: Array.isArray(data.fechas_reprogramacion)
      ? data.fechas_reprogramacion.filter((fecha): fecha is string => typeof fecha === "string")
      : [],
    tipoReprogramacion: (data.tipo_reprogramacion as string) || "",
    fecha: formatFechaRelativa(ts),
    fechaSort: d ? d.getTime() : 0,
  };
};

export const cargarNotificacionesUsuario = async (
  uid: string,
...
export const rutaNotificacionesPorRol = (): string => {
  ...
  if (rol === "Director" || rol === "Jefa Suprema")
    return "/director/notificaciones";
