...
import { labelEstadoUI } from '@/lib/dashboardSolicitudes'
import {
  labelCausaFlexibilidad,
} from '@/lib/flexibilidadCatalogo'
import { labelMateriaPorCodigo, type MateriaRegistrada } from '@/lib/materias'
...
export function mapSnapSolicitudesDirector(
  snapSolicitudes: { docs: Array<{ id: string; data: () => Record<string, unknown> }> },
  snapFlex: { docs: Array<{ id: string; data: () => Record<string, unknown> }> },
  ...
  const flexData = snapFlex.docs.map((d) => {
    ...
    const cursoLabel = String(
      data.curso_label || labelMateriaPorCodigo(String(data.curso || ''), materias, String(data.curso || '')),
    )
    const causaLabel = String(
      data.justa_causa_label || labelCausaFlexibilidad(String(data.justa_causa || ''), String(data.justa_causa || '')),
    )
    return {
      id: d.id,
...
export async function fetchSolicitudesDirector(materias: MateriaRegistrada[]): Promise<SolicitudDirector[]> {
  await obtenerSesion()

  const emptySnap = { docs: [] } as QuerySnapshot<DocumentData>

  const readCollection = async (name: string): Promise<QuerySnapshot<DocumentData>> => {
    try {
