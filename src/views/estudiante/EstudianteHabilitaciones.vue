...
import { fetchMaterias, labelMateriaPorCodigo, type MateriaRegistrada } from '@/lib/materias'
...
const TIPO_SOLICITUD: TipoSolicitudEstudiante = 'habilitaciones'
...
const enviar = async () => {
  ...
  try {
    ...
    await addDoc(collection(db, 'habilitaciones'), {
    ...
    await notificarDirectores({
      titulo: 'Nueva solicitud de habilitación',
      mensaje: `${nombreEstudiante.value} solicitó habilitación para ${cursoLabel.value}.`,
      tipo: 'info',
    ...
    archivoNombre.value = ''
    currentStep.value = 1
    await dialog.alert('Solicitud de habilitación enviada correctamente.', { variant: 'success', title: 'Listo' })
  } catch (error) {
    console.error('Error al enviar habilitación:', error)
    ...
    <div class="sol-layout">
      <aside class="sol-sidebar">
        <section class="sol-pendientes">
          ...
          <p v-else class="sol-pendientes-empty">No tienes solicitudes de habilitaciones.</p>
        </section>
      </aside>
          ...
          <div class="sol-form-body">
            <div v-if="currentStep === 1" class="sol-step-content">
              <div class="sol-step-header">
                ...
                <p>Elige la materia a habilitar; la información del curso se mostrará automáticamente</p>
              </div>
