...
import { fetchSolicitudesDocente, type SolicitudDocente } from '@/lib/docenteSolicitudes'
import {
  fetchMaterias,
...
import { obtenerSesion, leerNombreLocal } from '@/lib/session'
...
const badgeClass = (estadoClass: string) => {
  if (estadoClass.includes('aprobada') || estadoClass.includes('cerrada')) return 'approved'
...
const stats = computed(() => ({
  ...
  pendientes: solicitudes.value.filter(s => s.estadoLabel === 'Pendiente' || s.estadoLabel === 'En revisión').length,
  aprobadas: solicitudes.value.filter(s => s.estadoLabel === 'Aprobada' || s.estadoLabel === 'Cerrada').length,
  rechazadas: solicitudes.value.filter(s => s.estadoLabel === 'Rechazada').length,
}))
  ...
  <div class="docente-dashboard">
    ...
    <section class="hero-welcome">
      <div class="hero-content">
        ...
        <p class="hero-subtitle">Gestiona ausencias, reprogramaciones y tu carga académica</p>
        ...
        <router-link to="/docente/crear-solicitud" class="btn-hero-cta">
          ...
          <span class="btn-hero-text">
            ...
            <span class="btn-hero-hint">Reporta una ausencia y propón reprogramación</span>
          </span>
    ...
    <section class="acciones-grid">
      ...
      >
        <div class="accion-icon" :style="{ background: accion.bg, color: accion.color }">
          ...
          <svg v-else-if="accion.icon === 'plus'" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <line x1="12" y1="5" x2="12" y2="19"/>
            <line x1="5" y1="12" x2="19" y2="12"/>
          </svg>
    ...
    <div class="main-grid">
      <div class="card card-main">
        ...
        <div v-else-if="solicitudes.length === 0" class="empty-state">
          ...
          <p class="empty-desc">Usa «Nueva solicitud» para reportar una ausencia.</p>
        </div>
        ...
        <div v-else class="solicitudes-lista">
          ...
          >
            ...
            <div class="sol-info">
              ...
              <p class="sol-meta">{{ sol.tipoLabel }} · {{ formatFecha(sol.fecha) }} · {{ sol.reprogramacionTexto }}</p>
              <p v-if="sol.motivo_rechazo && sol.estadoLabel === 'Rechazada'" class="sol-rechazo">{{ sol.motivo_rechazo }}</p>
            </div>
            <span :class="['badge', badgeClass(sol.estadoClass)]">{{ sol.estadoLabel }}</span>
          </router-link>
        </div>

        <div v-if="solicitudes.length > 6" class="card-footer">
          <span>Mostrando 6 de {{ solicitudes.length }} solicitudes</span>
          <router-link to="/docente/mis-solicitudes" class="ver-link">Ver todas →</router-link>
        </div>
      </div>
      ...
      <div class="side-col">
        <div class="card">
          <div class="card-header">
            ...
            <router-link to="/docente/materias-asignadas" class="ver-link">Ver todas →</router-link>
          </div>
          ...
          <div v-else-if="materiasPreview.length === 0" class="empty-state pequeño">
            <p>No tienes materias asignadas.</p>
          </div>

          <div v-else class="materias-lista">
            <div v-for="materia in materiasPreview" :key="materia.id" class="materia-row">
...
.btn-hero-text { display: flex; flex-direction: column; gap: 2px; flex: 1; min-width: 0; }
.btn-hero-label { font-size: 14px; font-weight: 600; color: var(--color-text); }
.btn-hero-hint { font-size: 12px; color: var(--color-text-muted); }
.btn-hero-badge {
  font-size: 11px;
  font-weight: 600;
  padding: 4px 10px;
...
.btn-hero-arrow { color: var(--color-text-muted); flex-shrink: 0; transition: transform var(--transition); }
.btn-hero-cta:hover .btn-hero-arrow { transform: translateX(3px); color: var(--color-text); }
...
.accion-text { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px; }
.accion-title { font-size: 14px; font-weight: 600; color: var(--color-text); }
.accion-desc { font-size: 12px; color: var(--color-text-muted); }
.accion-arrow { color: var(--color-text-muted); flex-shrink: 0; }
...
.stat-tag-warn { color: var(--color-warning); background: var(--color-warning-bg); }
.stat-value {
  font-size: 36px;
  font-weight: 700;
...
.ver-link { font-size: 12px; color: var(--color-accent); font-weight: 500; text-decoration: none; }
...
.sol-rechazo { font-size: 11px; color: var(--color-error); margin-top: 4px; line-height: 1.35; }
.badge {
  font-size: 11px;
  font-weight: 500;
  padding: 3px 10px;
...
.materias-lista { display: flex; flex-direction: column; }
.materia-row {
  display: flex;
...
.materia-nombre { font-size: 13px; font-weight: 500; color: var(--color-text); }
.materia-meta { font-size: 11px; color: var(--color-text-muted); margin-top: 2px; }
...
.acceso:hover { background: var(--color-subtle); color: var(--color-text); }
.acceso-icon {
  width: 36px;
  height: 36px;
  border-radius: var(--radius);
...
.loading-state { padding: 20px; display: flex; flex-direction: column; gap: 10px; }
.skeleton {
  height: 52px;
  border-radius: var(--radius);
  background: linear-gradient(90deg, var(--color-subtle) 25%, var(--color-border-light) 50%, var(--color-subtle) 75%);
...
.empty-state.pequeño { padding: 24px 20px; }
