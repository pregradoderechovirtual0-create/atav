...
import { esperarAuth } from '@/lib/autenticacion/session'
...
const cargar = async (uid: string) => {
  ...
  try {
    ...
    console.error('Error cargando solicitudes docentes:', error)
    solicitudes.value = []
    errorCarga.value = 'No pudimos cargar tus solicitudes. Intenta actualizar la página.'
  } finally {
    cargando.value = false
  }
}

const cargarDesdeSesion = async () => {
  await esperarAuth()
  const user = auth.currentUser
  if (user) await cargar(user.uid)
  else cargando.value = false
}

onMounted(() => {
  void cargarDesdeSesion()
})

useRefreshOnVisible(cargarDesdeSesion)
...
<template>
  <div class="mis-solicitudes role-page">
    ...
    <div v-else-if="errorCarga" class="empty-state error-state">
      {{ errorCarga }}
    </div>
