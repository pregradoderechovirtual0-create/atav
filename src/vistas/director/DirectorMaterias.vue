...
const guardarMateria = async () => {
  const { codigo, nombre, semestre, dia, hora, profesor, enlace_reunion } =
    formMateria.value;
  ...
  try {
    if (modalModo.value === "crear") {
      ...
      if (yaExiste) {
        await dialog.alert(`Ya existe una materia con el código "${codigo}"`, {
          variant: "error",
        });
        return;
      }
      ...
      await updateDoc(doc(db, "materias", materiaEditando.value.id), {
        codigo,
        nombre,
        semestre,
        dia,
...
const confirmarEliminacion = async () => {
  ...
  try {
    ...
    materias.value = materias.value.filter(
      (m) => m.id !== materiaAEliminar.value.id,
    );
    mostrarToast();
  } catch (e) {
    console.error(e);
...
const mostrarToastImport = (cantidad: number) => {
  ...
  toastImportTimeout = setTimeout(() => {
    toastImportVisible.value = false;
  }, 4000);
};
...
const mostrarToastGuardado = () => {
  ...
  toastGuardadoTimeout = setTimeout(() => {
    toastGuardadoVisible.value = false;
  }, 3000);
};
...
const importarArchivo = async (event: Event) => {
  ...
  for (let i = 0; i < filas.length; i++) {
    ...
    if (codigosEnArchivo.has(codigo)) {
      errores.push(
        `Fila ${numFila}: el código "${codigo}" está duplicado en el archivo`,
      );
    } else {
    ...
    if (!diasValidos.includes(dia)) {
      errores.push(
        `Fila ${numFila}: el día "${dia}" no es válido (usa Lunes, Martes, Miércoles, Jueves, Viernes o Sábado)`,
      );
    }

    if (!/^\d{1,2}:\d{2}$/.test(hora)) {
      errores.push(
        `Fila ${numFila}: la hora "${hora}" no tiene formato válido (usa HH:MM, ej: 08:00)`,
      );
    }
    ...
    if (isNaN(sem) || sem < 1 || sem > 10) {
      errores.push(
        `Fila ${numFila}: el semestre "${semestre}" no es válido (debe ser entre 1 y 10)`,
      );
    }
  }
...
<template>
  <div class="director-list-page">
        ...
        <div class="table-card-header">
          <div>
            ...
            <p class="table-subtitle">
              {{ materiasFiltradas.length }} resultado{{
                ...
                <td>
                  <div class="row-actions">
                    <button
                      ...
                      aria-label="Editar"
                      title="Editar"
                      @click.stop="abrirModalEditar(materia)"
                    >
                      <svg
                        width="16"
                        height="16"
          ...
          <div class="modal-card modal-form" role="dialog" aria-modal="true">
              ...
              <div class="modal-top-row">
                ...
                <div class="modal-top-text">
                  <h2 class="modal-title">
                    {{
                      modalModo === "crear" ? "Nueva materia" : "Editar materia"
                    }}
                  </h2>
            ...
            <div class="modal-body">
              <section class="modal-section">
                ...
                <div class="modal-fields-grid">
                  <div class="field-group">
                    <label class="field-label" for="materia-codigo"
                      >Código</label
                    >
                    <input
                      id="materia-codigo"
                  ...
                  <div class="field-group">
                    <label class="field-label" for="materia-semestre"
                      >Semestre</label
                    >
                    <div class="select-wrapper">
                      <select
                        id="materia-semestre"
                        v-model="formMateria.semestre"
                  ...
                  <div class="field-group field-group-full">
                    <label class="field-label" for="materia-nombre"
              ...
              <section class="modal-section">
                ...
                <div class="field-group field-group-full">
                  <label class="field-label" for="materia-profesor"
                    >Profesor</label
                  >
                  <div class="select-wrapper">
                    <select
                      id="materia-profesor"
                      v-model="formMateria.profesor"
                      class="field-input field-select"
                    >
                ...
                <div class="field-group field-group-full">
                  <label class="field-label"> Enlace reunión virtual </label>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div
          v-if="modalErroresVisible"
          class="modal-overlay"
          @click.self="modalErroresVisible = false"
        >
          <div class="modal-card modal-form" role="dialog" aria-modal="true">
            <div class="modal-top">
              <div class="modal-top-row">
                ...
                <div class="modal-top-text">
                  ...
                  <p class="modal-subtitle">
                    No se importó ninguna materia. Corrige los siguientes
                    errores:
                  </p>
                </div>
                <button
                  ...
                  class="modal-close"
                  aria-label="Cerrar"
                  @click="modalErroresVisible = false"
                >
                  <svg
                    width="18"
                    height="18"
            ...
            <div class="modal-body modal-body--compact">
              <ul class="error-list">
                <li
                  v-for="(error, i) in erroresImport"
                  :key="i"
                  class="error-item"
                >
                  <svg
