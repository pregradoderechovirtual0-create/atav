...
const cargarUsuarios = async () => {
  ...
  try {
    ...
    usuarios.value = []
    errorCarga.value =
      'No se pudieron cargar los usuarios. Cierra sesión, entra con una cuenta de director y vuelve a intentar.'
  } finally {
    loading.value = false
  }
}
    ...
    <section class="table-section">
      ...
      <div v-else class="table-card">
        ...
        <div v-if="errorCarga" class="empty-panel">
          ...
          <p class="empty-desc">{{ errorCarga }}</p>
          <button type="button" class="btn btn-primary btn-sm" @click="cargarUsuarios">Reintentar</button>
        </div>

        <div v-else-if="usuariosFiltrados.length === 0" class="empty-panel">
