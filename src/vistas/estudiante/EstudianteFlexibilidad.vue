...
import { cargarContactoPerfil } from "@/lib/dominio/perfilContacto";
...
const justaCausaValida = computed(() => {
  ...
  if (formData.value.justaCausa === "otra")
    return formData.value.justaCausaDetalle.trim().length > 0;
  return true;
});
...
const ventanaActual = computed(() => {
  ...
  return getVentanaSeleccion(
    getRangoParcial(configParciales.value, formData.value.parcial),
  );
});
...
const diasCalendario = computed<(CeldaCalendario | null)[]>(() => {
  ...
  for (let d = 1; d <= totalDias; d++) {
    ...
    celdas.push({
      dia: d,
      iso,
      disponible: esDisponible(iso),
      festivo: esFestivo(iso),
    });
  }
  return celdas;
});
...
const datosEstudianteListos = computed(
  () =>
    nombreEstudiante.value.trim().length > 0 &&
    cedulaEstudiante.value.trim().length > 0,
);
...
const canGoNext = computed(() => {
  ...
  if (currentStep.value === 2)
    return !!formData.value.curso && materias.value.length > 0;
  ...
  if (currentStep.value === 5)
    return (
      !!formData.value.fechaSeleccionada && !!formData.value.horaSeleccionada
    );
  if (currentStep.value === 6)
    return !!formData.value.archivo && !!formData.value.descripcion;
  return true;
});
...
const nextStep = () => {
  if (currentStep.value < totalSteps && canGoNext.value) currentStep.value++;
};
const prevStep = () => {
...
const enviar = async () => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Flexibilización guardada; falló notificación:",
        notifError,
      );
      await dialog.alert(
...
const solicitarEnvio = async () => {
  if (!formularioCompleto.value) {
    if (!datosEstudianteListos.value) {
      await dialog.alert(
        "No encontramos tu nombre o cédula en el sistema. Contacta a la institución.",
        { variant: "error" },
      );
    }
    return;
...
const cargarSolicitudes = async (uid: string) => {
  ...
  solicitudesPendientes.value = snap.docs
    .map((docSnap) => {
      ...
      return {
        ...
        justa_causa:
          d.justa_causa_label || labelCausaFlexibilidad(d.justa_causa),
        correo: d.correo || "",
        celular: d.celular || "",
...
const iniciarSolicitudesEstudiante = async (uid: string) => {
  const cached = leerCacheSolicitudes<SolicitudFlexEstudiante>(
    uid,
    TIPO_SOLICITUD,
  );
...
onMounted(async () => {
  ...
  cargarConfigParciales().then((c) => {
    configParciales.value = c;
  });
  cargarMaterias();
});
...
watch(
  ...
  (parcial) => {
    ...
    formData.value.horaSeleccionada = null;
    if (!parcial) return;
    const ventana = getVentanaSeleccion(
      getRangoParcial(configParciales.value, parcial),
    );
    if (ventana?.inicio) {
...
const pasoActualHint = computed(
  () => stepMeta[currentStep.value - 1]?.hint ?? "",
);
  ...
  <div class="sol-page sol-page--flex">
      ...
      <aside class="sol-sidebar">
        <section class="sol-pendientes">
          <div class="sol-pendientes-header">
            ...
            <span class="sol-pendientes-badge">{{
            ...
            >
              <div class="sol-pendiente-row">
                <div class="sol-pendiente-left">
                  ...
                  <span class="sol-pendiente-meta"
                    >Parcial {{ sol.parcial }} · {{ sol.fechaSolicitud }}</span
                  >
                </div>
              ...
              <div
                v-if="sol.motivo_rechazo && sol.estado === 'Rechazada'"
                class="sol-motivo-rechazo"
              >
          ...
          <p v-else class="sol-pendientes-empty">
        ...
        <section class="sol-form-card">
          <header class="sol-form-header">
            <div class="sol-form-intro">
              ...
              <span class="sol-form-step-count"
          ...
          <div class="sol-form-body">
            ...
            <div v-if="currentStep === 1" class="sol-step-content">
              <div class="sol-step-header">
                ...
                <p>
                  Selecciona el parcial que necesitas cambiar de fecha u hora
                </p>
              </div>
              <div class="sol-form-grid">
                <div class="sol-form-group full-width">
                  <label class="sol-form-label">Parcial</label>
                  <p v-if="configParciales.periodo" class="sol-periodo-hint">
                    Periodo académico {{ configParciales.periodo }}
                  </p>
                  <div class="sol-parcial-cards">
                    ...
                    >
                      ...
                      <span class="sol-parcial-card-status">{{
                        p.estado
                      }}</span>
                      <span
                        v-if="formData.parcial === p.id"
                        class="sol-parcial-card-check"
                        aria-hidden="true"
                      >
                        <svg
                          width="14"
            ...
            <div v-if="currentStep === 2" class="sol-step-content">
              ...
              <p v-if="cargandoMaterias" class="sol-pendientes-empty">
                Cargando materias...
              </p>
              <template v-else>
            ...
            <div v-if="currentStep === 3" class="sol-step-content">
              ...
              <div class="sol-opciones-grid">
                <button
                  ...
                  :class="[
                    'sol-opcion-card',
                    { selected: formData.justaCausa === causa.id },
                  ]"
              ...
              <div
                v-if="formData.justaCausa === 'otra'"
                class="sol-form-group"
                style="margin-top: 16px"
              >
                <label class="sol-form-label"
                  ><span class="sol-required">*</span> Describe la justa
                  causa</label
                >
                <textarea
              ...
              <div class="sol-form-grid">
                <div v-if="correoPerfil" class="sol-form-group full-width">
                  <label class="sol-perfil-pref">
                    <input
                      v-model="usarCorreoPerfil"
                      type="checkbox"
                      class="sol-perfil-pref-check"
                    />
                    <span class="sol-perfil-pref-text">
                      Usar mi correo guardado:
                      <strong>{{ correoPerfil }}</strong>
                    </span>
                  </label>
                </div>
                <div class="sol-form-group full-width">
                  ...
                  <p
                    v-if="
                      ...
                      !esCorreoInstitucionalUsc(formData.correoInstitucional)
                    "
                    class="sol-form-hint"
                    style="color: #dc2626"
                  >
                ...
                <div v-if="celularPerfil" class="sol-form-group full-width">
                  <label class="sol-perfil-pref">
                    <input
                      v-model="usarCelularPerfil"
                      type="checkbox"
                      class="sol-perfil-pref-check"
                    />
                    <span class="sol-perfil-pref-text">
                      Usar mi celular guardado:
                      <strong>{{ celularPerfil }}</strong>
                    </span>
                  </label>
                </div>
                <div class="sol-form-group full-width">
            ...
            <div v-if="currentStep === 5" class="sol-step-content">
              ...
              <div v-if="ventanaTexto" class="ventana-info">
                <span v-if="configParciales.periodo" class="periodo-badge">{{
                  configParciales.periodo
                }}</span>
                Parcial {{ formData.parcial }}: puedes elegir del
                <strong>{{ ventanaTexto }}</strong> (días hábiles)
                <span v-if="configParciales.notas" class="periodo-notas">{{
                  configParciales.notas
                }}</span>
              </div>
              <div v-else class="ventana-info ventana-warning">
                Las fechas del Parcial {{ formData.parcial }} aún no están
                configuradas.
              </div>
              ...
              <div class="cal-wrapper">
                <div class="cal-nav">
                  ...
                  <span class="cal-mes-label"
                    >{{ mesesNombres[calMes] }} {{ calAnio }}</span
                  >
                  <button
                    class="cal-nav-btn"
                    type="button"
                  ...
                  <div
                    ...
                    :class="[
                      'cal-celda',
                      {
                        ...
                        nodisponible:
                          celda && !celda.disponible && !celda.festivo,
                        festivo: celda && celda.festivo,
                        seleccionado:
                          celda && formData.fechaSeleccionada === celda.iso,
                      },
                    ]"
                ...
                <div class="cal-leyenda">
                  <span class="ley-item"
                    ><span class="ley-dot disponible"></span> Disponible</span
                  >
                  <span class="ley-item"
                    ><span class="ley-dot nodisponible"></span> No
                    disponible</span
                  >
                  <span class="ley-item"
                    ><span class="ley-dot festivo-c"></span> Festivo</span
                  >
                </div>
              </div>
              ...
              <div v-if="formData.fechaSeleccionada" class="horarios-wrap">
                <p class="horarios-titulo">
                  ...
                  <strong>{{ formatFecha(formData.fechaSeleccionada) }}</strong>
                </p>
                ...
                <div class="horarios-grupo">
                  ...
                  <div class="horarios-grid">
                    <button
                      v-for="h in HORARIOS_MANANA"
                      :key="h"
                      type="button"
                      :class="[
                        'hora-btn',
                        { selected: formData.horaSeleccionada === h },
                      ]"
              ...
              <div
                v-if="formData.fechaSeleccionada && formData.horaSeleccionada"
                class="fecha-seleccionada-info"
              >
                <svg
                  width="15"
                  height="15"
                ...
                Seleccionaste:
                <strong>{{ formatFecha(formData.fechaSeleccionada) }}</strong> a
                las <strong>{{ formatHora(formData.horaSeleccionada) }}</strong>
              </div>
            </div>
            ...
            <div v-if="currentStep === 6" class="sol-step-content">
              <div class="sol-step-header">
                ...
                <p>
                  Adjunta la evidencia en PDF y describe brevemente tu solicitud
                </p>
              </div>
              <div class="sol-form-grid">
                <div class="sol-form-group full-width">
                  <label class="sol-form-label"
                    ...
                    <span class="sol-label-hint">(solo PDF)</span></label
                  >
                  <PdfUploadArea
          ...
          <footer class="sol-form-footer">
            ...
            <div class="sol-form-footer-actions">
              <router-link to="/estudiante" class="sol-btn sol-btn-ghost"
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div
          v-if="modalDetalleVisible && solicitudDetalle"
          class="modal-overlay"
          ...
          <div class="modal-detalle-card">
            ...
            <div class="modal-detalle-body">
              <div class="detalle-estado-row">
                ...
                >
                <span class="detalle-fecha-sol"
                  >Solicitada el {{ solicitudDetalle.fechaSolicitud }}</span
                >
              </div>
                ...
                <div class="detalle-grid">
                  <div class="detalle-item">
                    ...
                    <span class="detalle-value">{{
                      solicitudDetalle.materia
                    }}</span>
                  </div>
                  <div class="detalle-item">
                    <span class="detalle-label">Parcial</span>
                    <span class="detalle-value"
                      >Parcial {{ solicitudDetalle.parcial }}</span
                    >
                  </div>
                </div>
              </div>
              ...
              <div class="detalle-block">
                ...
                <div class="detalle-grid">
                  <div class="detalle-item">
                    ...
                    <span class="detalle-value">{{
                      formatFechaParcial(solicitudDetalle.fecha_parcial)
                    }}</span>
                  </div>
                  <div class="detalle-item">
                    <span class="detalle-label">Hora</span>
                    <span class="detalle-value">{{
                      formatHoraParcial(solicitudDetalle.hora_parcial)
                    }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="detalle-block">
                ...
                <div class="detalle-grid">
                  <div class="detalle-item">
                    ...
                    <span class="detalle-value">{{
                      solicitudDetalle.correo || "—"
                    }}</span>
                  </div>
                  <div class="detalle-item">
                    <span class="detalle-label">Celular</span>
                    <span class="detalle-value">{{
                      solicitudDetalle.celular || "—"
                    }}</span>
                  </div>
                </div>
              </div>
              ...
              <div v-if="solicitudDetalle.pdf_url" class="detalle-block">
                ...
                <a
                  :href="solicitudDetalle.pdf_url"
                  target="_blank"
                  class="detalle-pdf-link"
                >
              ...
              <div
                ...
                "
                class="detalle-rechazo"
              >
                <h3 class="detalle-block-title">Motivo de rechazo</h3>
                <p class="detalle-rechazo-texto">
                  {{ solicitudDetalle.motivo_rechazo }}
                </p>
              </div>
            </div>
    ...
    <Teleport to="body">
      <Transition name="toast">
        <div
          ...
          :class="[
            'toast',
            toastTipo === 'error' ? 'toast-error' : 'toast-success',
          ]"
        >
          <div class="toast-icon">
            <svg
              v-if="toastTipo === 'success'"
              width="18"
              height="18"
              viewBox="0 0 24 24"
              fill="none"
      ...
      <div v-if="subiendoPdf" class="loading-overlay">
        <div class="loading-box">
          ...
          <span class="loading-hint"
            >Por favor espera, no cierres la página</span
          >
        </div>
      </div>
    </Teleport>
