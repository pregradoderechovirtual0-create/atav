...
<template>
  <div class="encuentros-page role-page">
    <div class="page-header">
      ...
      <p>Accede a tus clases virtuales del semestre</p>
    </div>

    <p v-if="cargando" class="empty-state">Cargando encuentros...</p>

    <p v-else-if="error" class="empty-state">
      {{ error }}
    </p>

    <div v-else-if="materias.length" class="encuentros-grid">
      <article
    ...
    <p v-else class="empty-state">No tienes materias suscritas actualmente.</p>

    <!-- MODAL -->

    <div v-if="mostrarAviso" class="modal-overlay" @click.self="cerrarAviso">
      <div class="modal-aviso">
        ...
        <h3>Reunión no disponible</h3>

        <p>
          {{ mensajeAviso }}
        </p>

        <button class="modal-btn" @click="cerrarAviso">Entendido</button>
      </div>
    </div>
  </div>
</template>
